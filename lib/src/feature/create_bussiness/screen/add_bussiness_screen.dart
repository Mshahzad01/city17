  ...
  Widget build(BuildContext context) {
    return BlocConsumer<AddBussinessCubit, AddBussinessState>(
      listener: (context, state) {
        if (state is AddBussinessLoaded && (state.hasError ?? false)) {
          ToastUtils.errorToast(state.message.toString());
        }

        if (state is AddBussinessLoaded && (state.loaded ?? false)) {
          ToastUtils.succesToast('Bussiness Create Successfuly');

          BlocProvider.of<BussinessCubit>(context).getBussinesData();
